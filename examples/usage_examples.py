...
async def gene_search_example():
    ...
    async with server:
        try:
            ...
            result = await server.execute_request(
                "search_genes",
                {"term": "BRCA", "species": "human", "max_results": 5}
            )
            ...
            for i, gene in enumerate(result['results'], 1):
                print(f"{i}. {gene.get('gene_id', 'N/A')} - {gene.get('description', 'N/A')}")
...
async def batch_operations_example():
    ...
    async with server:
        try:
            ...
            result = await server.execute_request(
                "batch_gene_info",
                {"gene_ids": ["TP53", "BRCA1", "EGFR", "MYC", "KRAS"], "species": "human"}
            )
...
async def genomic_region_search_example():
    ...
    async with server:
        try:
            ...
            print(f"搜索区域: 染色体 {result['results'][0].get('summary', {}).get('chromosome', 'N/A')}")
            print(f"总结果数: {result['total_count']}")
    ...
    async with server:
        try:
            ...
            tasks = [
                ...
                server.execute_request("search_genes", {"term": "cancer", "max_results": 5}),
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    ...
                    operation = ["TP53信息", "BRCA1信息", "癌症基因搜索", "EGFR摘要", "MYC同源体"][i]
                    if "gene_id" in result:
                        print(f"✅ 请求 {i+1} ({operation}): {result['gene_id']}")
                    elif "term" in result:
                        print(f"✅ 请求 {i+1} ({operation}): {result['total_count']} 个结果")
                    else:
...
async def error_handling_example():
    ...
    async with server:
        ...
        # 2. 数据未找到错误
        try:
            result = await server.execute_request("get_gene_info", {"gene_id": "NONEXISTENT_GENE"})
        ...
        try:
            ...
            result = await server.execute_request("batch_gene_info", {"gene_ids": too_many_genes})
        except ValidationError as e:
...
async def performance_comparison_example():
    ...
    gene_ids = ["TP53", "BRCA1", "EGFR", "MYC", "KRAS", "AKT1", "PIK3CA", "PTEN", "RB1", "CDKN2A"]
    ...
    async with server:
        ...
        print(f"  完成 {len(individual_results)} 个请求，耗时: {individual_time:.2f} 秒")
        ...
        try:
            batch_result = await server.execute_request("batch_gene_info", {"gene_ids": gene_ids})
            batch_time = time.time() - start_time
            print(f"  完成 {batch_result['successful']} 个请求，耗时: {batch_time:.2f} 秒")
