...
class GenomeMCPCLI:
    ...
    def _build_batch_requests(self, args: argparse.Namespace) -> List[Dict[str, Any]]:
        ...
        if args.operation == "batch_gene_info":
            ...
            raise ValidationError(f"Batch operation not supported for: {args.operation}")
    ...
    def _format_pretty(self, result: Dict[str, Any]) -> str:
        ...
        # Header
        server_name = result.get('server', result.get('result', {}).get('server_name', 'Unknown'))
        ...
        if result.get('batch', False):
            ...
            for i, batch_result in enumerate(batch_results, 1):
                ...
                if batch_result.get('success', False):
                    ...
                    output.append(f"   错误: {batch_result.get('error', 'Unknown error')}")
                output.append("")
        else:
            # Single result
            ...
            if result['operation'] == 'server_info':
                ...
                output.append(f"🔄 批量支持: {'✅' if data.get('supports_batch', False) else '❌'}")
                ...
                if 'stats' in data:
                    ...
                    output.append(f"   平均响应时间: {stats.get('avg_response_time', 0):.3f} 秒")
            ...
            if 'results' in data and data['results']:
                ...
                for i, search_result in enumerate(data['results'][:5], 1):  # Show first 5
                    output.append(f"   {i}. {search_result.get('gene_id', 'N/A')} - {search_result.get('description', 'N/A')}")
            ...
            if 'summary' in data and data['summary']:
                output.append("📄 摘要:")
                summary_text = data['summary'][:300] + "..." if len(data['summary']) > 300 else data['summary']
            ...
            if 'homologs' in data and data['homologs']:
                ...
                for homolog in data['homologs'][:5]:  # Show first 5
                    output.append(f"   • {homolog.get('species', 'N/A')}: {homolog.get('gene_id', 'N/A')}")
...
示例:
  ...
  %(prog)s ncbi-gene search-by-region --chromosome 17 --start 43044295 --end 43125483
    ...
    # Gene info operation
    gene_info_parser = subparsers.add_parser(
        "get_gene_info",
        help="获取基因详细信息"
    )
    ...
    homologs_parser = subparsers.add_parser(
        "get_gene_homologs",
        help="获取基因同源体"
    )
