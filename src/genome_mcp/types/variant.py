...
class GenomicPosition(BaseModel):
    ...
    @validator('ref_allele', 'alt_allele')
...
class FunctionalPrediction(BaseModel):
    ...
    confidence: ConfidenceLevel = Field(ConfidenceLevel.UNKNOWN, description="Prediction confidence")
    source: DataSource = Field(..., description="Data source")
...
class VariantInfo(BaseModel):
    ...
    clinical_significance: List[ClinicalSignificance] = Field(default_factory=list, description="Clinical significance")
    population_frequencies: List[PopulationFrequency] = Field(default_factory=list, description="Population frequencies")
    functional_predictions: List[FunctionalPrediction] = Field(default_factory=list, description="Functional predictions")
    genes: List[str] = Field(default_factory=list, description="Affected genes")
    consequences: List[str] = Field(default_factory=list, description="Variant consequences")
    related_variants: List[str] = Field(default_factory=list, description="Related variants")
...
class VariantResponse(BaseModel):
    ...
    alternate_ids: List[str] = Field(default_factory=list, description="Alternate variant IDs")
...
class RegionVariantQuery(BaseModel):
    ...
    assembly: str = Field("GRCh38", description="Genome assembly version")
    variant_types: List[str] = Field(default_factory=list, description="Variant types to include")
    min_frequency: Optional[float] = Field(None, ge=0.0, le=1.0, description="Minimum allele frequency")
    max_frequency: Optional[float] = Field(None, ge=0.0, le=1.0, description="Maximum allele frequency")
    ...
    def validate_positions(cls, start, end, values):
        ...
        if end - start > 1000000:  # 1MB limit
...
class RegionVariantResponse(BaseModel):
    ...
    variants: List[VariantInfo] = Field(default_factory=list, description="Found variants")
