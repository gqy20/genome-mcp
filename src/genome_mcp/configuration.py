...
class RateLimitConfig(BaseModel):
    ...
    requests_per_hour: int = Field(3600, ge=1, le=100000, description="Requests per hour")
...
class APIConfig(BaseModel):
    ...
    retry_delay: float = Field(1.0, ge=0.1, le=10.0, description="Delay between retries in seconds")
    user_agent: str = Field("Genome-MCP/1.0.0", description="User agent string")


class NCBIConfig(BaseModel):
    """NCBI-specific configuration."""
    base_url: str = Field("https://api.ncbi.nlm.nih.gov", description="NCBI API base URL")
    api_key: Optional[str] = Field(None, description="NCBI API key")
    max_batch_size: int = Field(100, ge=1, le=500, description="Maximum batch size for requests")
    rate_limit_delay: float = Field(0.1, ge=0.0, le=1.0, description="Rate limit delay in seconds")


class EnsemblConfig(BaseModel):
    """Ensembl-specific configuration."""
    base_url: str = Field("https://rest.ensembl.org", description="Ensembl API base URL")
    server_url: str = Field("https://www.ensembl.org", description="Ensembl server URL")
    max_batch_size: int = Field(1000, ge=1, le=5000, description="Maximum batch size for requests")
    rate_limit_delay: float = Field(0.05, ge=0.0, le=1.0, description="Rate limit delay in seconds")
...
class LoggingConfig(BaseModel):
    ...
    max_file_size: int = Field(10485760, ge=1048576, le=104857600, description="Max log file size in bytes")
