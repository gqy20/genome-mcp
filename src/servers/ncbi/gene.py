...
class NCBIGeneServer(BaseMCPServer):
            ...
            if homologene_ids:
                ...
                for homolog_id, homolog_info in homologs_data.items():
                    if homolog_id != "uids":
                        ...
                        if not target_species or target_species.lower() in homolog_species.lower():
                            homologs.append({
    ...
    async def _batch_gene_info(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not gene_ids or not isinstance(gene_ids, list):
            raise ValidationError("gene_ids must be a non-empty list", field_name="gene_ids")
    ...
    async def _search_by_region(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            position = GenomicDataParser.parse_genomic_position(f"{chromosome}:{start}-{end}")
        ...
        search_term = f"chr{chromosome}:{start}-{end}[Chromosome] AND {species}[Organism]"
