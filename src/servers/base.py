...
class ServerStats:
    ...
    def increment_success(self, response_time: float, bytes_sent: int = 0, bytes_received: int = 0):
    ...
    def rate_limiter(self) -> RateLimiter:
        ...
        if self._rate_limiter is None:
            self._rate_limiter = RateLimiter(
                ...
                requests_per_hour=self.capabilities.rate_limit_requests * 60 // self.capabilities.rate_limit_window
            )
        ...
        try:
            ...
            if use_cache and self.config.enable_caching:
                cache_key = generate_cache_key(
                    f"{self.capabilities.name}:{operation}",
                    **params
                )
