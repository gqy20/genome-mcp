...
def main():
    ...
    # Test gene search
    tests_total += 1
    if test_command(['--format', 'json', 'ncbi-gene', 'search_genes', '--term', 'TP53', '--max-results', '3'], "基因搜索"):
        tests_passed += 1
    
    # Test gene info
    tests_total += 1
    if test_command(['--format', 'json', 'ncbi-gene', 'get_gene_info', '--gene-id', 'TP53'], "基因信息"):
        tests_passed += 1
    
    # Test gene summary
    tests_total += 1
    if test_command(['--format', 'json', 'ncbi-gene', 'get_gene_summary', '--gene-id', 'TP53'], "基因摘要"):
        tests_passed += 1
    
    # Test batch gene info
    tests_total += 1
    if test_command(['--batch', '--format', 'json', 'ncbi-gene', 'batch_gene_info', '--gene-ids', 'TP53,BRCA1'], "批量基因信息"):
        tests_passed += 1
    
    # Test region search
    tests_total += 1
    if test_command(['--format', 'json', 'ncbi-gene', 'search_by_region', '--chromosome', '17', '--start', '43044295', '--end', '43125483', '--max-results', '5'], "区域搜索"):
        tests_passed += 1
