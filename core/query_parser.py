...
class QueryParser:
    ...
    def _parse_auto(query: str) -> ParsedQuery:
        ...
        # 进化生物学关键词检测
        evolution_keywords = [
            "homolog", "ortholog", "paralog", "conservation", "phylogen", "evolution",
            "comparative", "species", "across", "conserved", "family", "ancestral"
        ]
        if any(keyword in query.lower() for keyword in evolution_keywords):
            return QueryParser._parse_ortholog(query)
