...
from exceptions import ValidationError, DataNotFoundError, APIError
...
async def test_ncbi_gene_server():
    ...
    async with NCBIGeneServer(config) as server:
        try:
            result = await server.execute_request("search_genes", {
                "term": "BRCA",
                "species": "human",
                "max_results": 5
            })
    ...
    async with NCBIGeneServer(config) as server:
        try:
            result = await server.execute_request("batch_gene_info", {
    ...
    async with NCBIGeneServer(config) as server:
        try:
            result = await server.execute_request("search_by_region", {
                "chromosome": "17",
                "start": 43044295,
                "end": 43125483,
                "species": "human",
    ...
    async with NCBIGeneServer(config) as server:
        try:
            result = await server.execute_request("get_gene_homologs", {
                "gene_id": "TP53",
                "species": "human"
            })
    ...
    async with NCBIGeneServer(config) as server:
        try:
            result = await server.execute_request("get_gene_summary", {
                "gene_id": "TP53",
                "species": "human"
            })
    ...
    async with NCBIGeneServer(config) as server:
        try:
            ...
            result = await server.execute_request("get_gene_expression", {
                "gene_id": "TP53",
                "species": "human"
            })
    ...
    async with NCBIGeneServer(config) as server:
        try:
            ...
            result = await server.execute_request("get_gene_pathways", {
                "gene_id": "TP53",
                "species": "human"
            })
    ...
    async with NCBIGeneServer(config) as server:
        try:
            ...
            await server.execute_request("batch_gene_info", {
                "gene_ids": too_many_genes,
                "species": "human"
            })
            assert False, "Should have raised ValidationError"
        except ValidationError as e:
    ...
    async with NCBIGeneServer(config) as server:
        ...
        try:
            await server.execute_request("get_gene_info", {"gene_id": "TP53", "species": "human"})
            await server.execute_request("search_genes", {"term": "cancer", "species": "human", "max_results": 3})
